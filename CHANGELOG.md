...
- 修复 macOS 弹幕搜索无法用拼音输入中文的问题：mpv 的 mac 前端没有输入法
  管线（`--input-ime` 仅支持 Windows/Wayland），菜单内搜索框只能打英文。
  现在点「搜索弹幕」会先弹出系统输入框（原生支持拼音输入法），确认后直接
  搜索；点取消或清空则回退到菜单内输入，此时可用 Cmd+V 粘贴。
- 修复 Windows 与 macOS 菜单内复制粘贴：粘贴（Ctrl+V / Cmd+V）到未被编辑
  过的预填搜索词时整体替换，已编辑过则在光标处插入；搜索框处于输入状态时
  复制（Ctrl+C / Cmd+C）会复制当前输入的文本。
- 联机面板房主模式新增「复制共享地址」：一键把 AList 共享地址复制到剪贴板
  发给观看者。
