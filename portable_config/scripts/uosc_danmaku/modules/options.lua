...
--   1. 首次运行时使用的默认样式；
--   2. 样式菜单中"恢复默认配置"的目标值。
-- 注意：用户通过弹幕样式菜单修改过的值会写入 script-opts/uosc_danmaku.conf，
-- 并在这里被 read_options 覆盖，因此"修改一次即成为默认值"，无需每次重设。
...
-- 运行时样式配置表：先复制内置默认值，再加载外部配置文件进行覆盖
options = {}
for key, value in pairs(default_options) do
    options[key] = value
end

-- 内置默认值（只读），供样式菜单判断"是否已被修改"以及恢复默认使用
