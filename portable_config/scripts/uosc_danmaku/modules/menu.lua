...
-- 内置默认值（来自 modules/options.lua），用于「恢复默认」以及判断某项样式是否已被修改
local builtin_defaults = DEFAULT_OPTIONS or options
local style_store = require("modules/style")

local menu_items_config = {
    bold = { title = "粗体", hint = options.bold, original = builtin_defaults.bold,
        footnote = "true / false", },
    fontsize = { title = "大小", hint = options.fontsize, original = builtin_defaults.fontsize,
        scope = { min = 0, max = math.huge }, footnote = "请输入整数(>=0)", },
    outline = { title = "描边", hint = options.outline, original = builtin_defaults.outline,
        scope = { min = 0.0, max = 4.0 }, footnote = "输入范围：(0.0-4.0)" },
    shadow = { title = "阴影", hint = options.shadow, original = builtin_defaults.shadow,
        scope = { min = 0, max = math.huge }, footnote = "请输入整数(>=0)", },
    scrolltime = { title = "速度", hint = options.scrolltime, original = builtin_defaults.scrolltime,
        scope = { min = 1, max = math.huge }, footnote = "请输入整数(>=1)", },
    opacity = { title = "透明度", hint = options.opacity, original = builtin_defaults.opacity,
        scope = { min = 0, max = 1 }, footnote = "输入范围：0（完全透明）到1（不透明）", },
    displayarea = { title = "弹幕显示范围", hint = options.displayarea, original = builtin_defaults.displayarea,
        scope = { min = 0.0, max = 1.0 }, footnote = "显示范围(0.0-1.0)", },
}

-- 判断某项样式当前是否已被修改（菜单里的值可能是数字也可能是字符串，统一转成字符串比较）
local function is_style_modified(config)
    return tostring(config.hint) ~= tostring(config.original)
end

-- 保存用户修改的样式：写入 script-opts/uosc_danmaku.conf，下次启动继续生效
local function save_style(key)
    if not style_store then
        return
    end
    style_store.set(key, options[key])
end

-- 恢复某项样式的内置默认值，并删除配置文件里的覆盖项
local function reset_style(key)
    if not style_store then
        return
    end
    style_store.clear(key)
end

-- 让不需要重新加载弹幕数据的样式（描边/阴影/透明度/显示范围/粗体）立刻生效。
-- 暂停时渲染循环是停止的，这里主动重绘一次，保证暂停状态下也能看到样式变化。
local function refresh_style_render(key)
    if key == "fontsize" or key == "scrolltime" then
        return
    end
...
function open_style_menu_get(query, indicator)
    ...
    local function build_menu(source)
        ...
        for _, key in ipairs(ordered_keys) do
            ...
            text = is_style_modified(config) and text .. "⟳" or text
        ...
        if select_num == 0 then
            ...
            local suffix = ""
            if config and is_style_modified(config) then
                suffix = "（输入\\r恢复默认配置）"
            end
...
function open_style_menu_uosc(actived, status)
    ...
    for _, key in ipairs(ordered_keys) do
        ...
        if is_style_modified(config) then
