...
---@alias MenuData {id?: string; type?: string; title?: string; hint?: string; footnote: string; search_style?: 'on_demand' | 'palette' | 'disabled';  item_actions?: MenuAction[]; item_actions_place?: 'inside' | 'outside'; callback?: string[]; keep_open?: boolean; persistent?: boolean; bold?: boolean; italic?: boolean; muted?: boolean; separator?: boolean; align?: 'left'|'center'|'right'; items?: MenuDataChild[]; selected_index?: integer; on_search?: string|string[]; on_paste?: string|string[]; on_move?: string|string[]; on_close?: string|string[]; search_debounce?: number|string; search_submenus?: boolean; search_suggestion?: string; search_submit?: boolean; bind_keys?: string[]}
...
function Menu:handle_cursor_down()
	...
	elseif not self.root.persistent then
		self:close()
	end
end
...
function Menu:render()
	...
	-- The main panel is persistent so accidental left-clicks, Escape, and
	-- clicks outside do not dismiss it.  Preserve the expected right-click
	-- gesture, but only when the pointer is outside the current menu rectangle.
	if self.type == 'menu' then
		cursor:zone('secondary_down', display, self:create_action(function()
			if self.proximity_raw > 0 then self:close() end
		end))
	end
