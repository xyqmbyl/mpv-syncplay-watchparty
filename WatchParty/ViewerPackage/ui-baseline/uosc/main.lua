...
defaults = {
	...
	controls =
	'menu,ST-stats_tog,ST-thumb_tog,gap,play_pause,gap,subtitles,audio,<has_chapter>chapters,<has_many_edition>editions,<has_many_video>video,<stream>stream-quality,gap,space,speed,space,command:search:script-message open_search_danmaku_menu?搜索弹幕,command:add_box:script-message open_add_source_menu?从源添加弹幕,command:grid_view:script-message open_add_total_menu?弹幕设置,cycle:toggle_on:show_danmaku@uosc_danmaku:on=toggle_on/off=toggle_off?弹幕开关,shuffle,loop-playlist,loop-file,gap,prev,items,next,gap,fullscreen',
	controls_size = 32,
	controls_margin = 8,
	controls_spacing = 2,
