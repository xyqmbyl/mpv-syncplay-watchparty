...
仅允许 Tailscale `100.64.0.0/10` 访问 AList 5244 的防火墙规则。第一次写入这条
规则会触发一次 UAC 管理员授权（`runas`），请在弹出「用户账户控制」时点「是」，
否则配置会中断；规则已经存在时面板会直接复用、不再重复弹窗，所以再次切换房主
模式是秒级完成的。如果 mpv 已经在管理员权限下运行就不会弹窗。不要在路由器上给
5244 做端口映射。

同一台电脑上如果解压了多份 WatchParty 副本，请只用其中一份：5244 端口只能被
一个 AList 占用。若点击房主模式时发现端口已被**另一份副本**的 `alist.exe` 占着，
向导会停掉那个进程并改用当前目录的 AList（使用当前目录的密码与 `media`），
否则会出现「AList 管理员登录失败」或共享到另一个副本的视频目录。被其它程序
占用时仍然只提示、不处理；查不到占用进程时保守复用，不会误杀进程。
...
2. 在面板里进入「联机」→「运行模式」→「观看者模式」，按提示输入房主发来的
   ...
   共享，并从观看者视角检测房主 AList 是否可达。如果你现在正处在房主模式，
   直接点「切换为观看者模式」不会生效——面板会提示先在「运行模式」菜单顶部的
   输入框里填入房主的 `100.x.x.x`，这是为了避免把本机地址当成房主地址。
3. 诊断通过后即可加入房间。
